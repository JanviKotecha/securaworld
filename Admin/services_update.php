...
<?php @include("include/secureConfig.php"); 
  ...
  if(isset($_POST['update'])) {
    if(!empty($_POST['lintyp']) && !empty($_POST['link']) && !empty($_POST['tit']) && !empty($_POST['desc'])) {
      ...
      $qm->updateRecord("services","tit='".$tit."',des='".$des."',link='".$link."',lintyp='".$lintyp."'","id=".$id);
      ...
      if(isset($_FILES['image']['tmp_name']) && !empty($_FILES['image']['tmp_name'])){
        ...
        if(in_array($ext,$allowd)) {
          $sel=$qm->getRecord("services","img","id=".$id);
          if(mysqli_num_rows($sel) > 0){
            $result=mysqli_fetch_array($sel);
            unlink(UPLOAD_SERVICES_URL.$result['img']);
          }
          move_uploaded_file($_FILES['image']['tmp_name'],UPLOAD_SERVICES_URL.$Img);
          $qm->updateRecord("services","img='".$Img."'","id=".$id);
          $_SESSION['alert_msg'] .= "<div class='msg_success'>services updated successfully.</div>";
  ...
  {
    $id=$_GET['id'];
    $res = $qm->getRecord("services","*","id=".$id);
            ...
            <div class="row page-title-header">
              <div class="col-12">
                <div class="page-header">
                  <h5><a href="services.php" style="text-decoration: none;color: black;">Services&nbsp;</a> <i class="fa fa-chevron-right"></i> Update Services </h5>
                ...
                <div class="card">
                  <div class="card-body">
                    <form class="form-sample" action="" method="Post" enctype="multipart/form-data"> 
                      <p class="card-description"><b><CENTER>Update Services</CENTER></p>
                      <input type="hidden" name="id" class="txtField" value="<?php echo $id; ?>">
                      <div class="row">
                        <div class="col-md-5">
                          <div class="form-group row">
                            <div class="col-sm-12">
                              <center><img src="<?php echo $row["img"]=='' ? SERVICES_URL.'noimg.png' : (file_exists(UPLOAD_SERVICES_URL.$row["img"]) ? SERVICES_URL.$row["img"] :  SERVICES_URL.'noimg.png'); ?>" style="width:100%"></center>
                            </div>
                          </div>
                        </div>
                        <div class="col-md-7">
